...
Description  
//Envia e-mail para usuario solicitantes retorno de ocorrências
...
Local cTo         := ""
Local cSubJect    := "Ocorrencia em produto " + aTabSC1[1]  
Local lEnviado    := .F.  
Local lSmtpAuth   := .T.
Local lOk         := .F. 
...
cEmailComp:=UsrRetMail(RetCodUsr(aTabSC1[6])) 
	...
	cBody := '<html>'
	cBody += '<head>'
	cBody += '<title> Retorno de Ocorrencias </title>'
	cBody += '</head>'  
	cBody += '<b><font size="3" face="Arial">Numero da Solicitação: ' + aTabSC1[1] + ' </font></b><br><br>'

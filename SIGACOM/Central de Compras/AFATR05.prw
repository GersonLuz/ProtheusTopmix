...
If dbSeek(cFilCot+cNumCot+cFornece+cLoja)
	...
	While !Eof("SC8") .And. SC8->C8_FILIAL == cFilCot .And. SC8->C8_NUM == cNumCot .And.;
		...
		cBody += '<tr><td>' + Alltrim(SC8->C8_ITEM) + '</td><td>' + Alltrim(SB1->B1_ZREF1) + '</td><td>' + Alltrim(SC8->C8_ZDESCRI) + '</td><td>' + Alltrim(SB1->B1_FABRIC) + '</td><td>'+ SC8->C8_UM + '</td><td>'+ Transform(SC8->C8_QUANT,"@E 9,999,999.99") + '</td><td></td><td></td></tr>'

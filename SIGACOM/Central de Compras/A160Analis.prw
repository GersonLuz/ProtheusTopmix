...
Case nEvento == 4
			...
			For nX := 1 To Len(aColsSCE)
				For nY := 1 To Len(aColsSCE[nX])
					If ( aVencedor[nZ][1]==aSC8[nX][nY][nPForSC8][2].And.;
						...
						SC7->C7_APROV   := cGrupo
						IIF(Empty(SC7->C7_USER),Alert("Não gravou usuário"),'') 
					   	MsUnLock()
						MaFisIniLoad(1)
	...
	Next nZ
	MsgInfo("Olá"+' '+CUserName+' '+' '+"-"+' '+"Pedidos Gerados"+' '+ Chr(13) + Chr(10) + cNPed, "TOPMIX")
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
If Empty(cPC)
 Alert("Pedido"+' '+cDocto+' '+ "sem usuário gravado, favor entrar em contato com o Cristiano da TI.")
Endif
	...
	If nOper == 5  //Estorno da Aprovacao
		...
		While !Eof() .And. SCR->CR_FILIAL+SCR->CR_TIPO+SCR->CR_NUM == xFilial("SCR")+cTipoDoc+cDocto
			...
			Else
				...
				//³residuos, em caso da opcao de estorno a reconstituicao do SCR ³

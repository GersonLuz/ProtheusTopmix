			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				Else
					...
					ElseIf lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
						...
						cObsTop := " EXECUCAO, POR ADM, EMPREITADA OU SUBEMP. OBRA CIVIL - Valor aproximado dos tributos: R$ " + ConvType(SF2->F2_TOTIMP,15,2)+"."
						cDiscrNFSe := @cNatOper +' '+cDiscrNFSe + cObsTop
						//cDiscrNFSe += "$$$"
						ElseIf SF2->(FieldPos("F2_MENNOTA")) > 0
					...
					If Len(aProd) > 0
						...
						If nX > 0
							...
							//aProd[nx][29]+=	SF3->F3_ISSSUB + SF3->F3_ISSMAT	//Valor Total de deducoes       Comentado para não duplicar o valor na tag ValorDeducoes
						    	//-- Valor total unitário aglutinado
							 if lAglutina .And. !((cAliasSD2)->D2_TIPO $ "IP") 
								if len (aProd) > 0
									aProd[nx][10] :=  aProd[nx][28] / aProd[nx][9] // Valor total unitário somado X quantidade unitário somado
								endif
							endif
						Else
							lAglutina := .F.

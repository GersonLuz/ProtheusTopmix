...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		//MAX ROCHA: 03-05-2012 --> CÓDIGO OBRA TOPMIX		
		dbSelectArea("SC5")
		dbSetOrder(1)
		If DbSeek(xFilial("SC5")+(cAliasSD2)->D2_PEDIDO)
			IF ! EMPTY(SC5->C5_OBRA)
		   	aadd(aConstr,(SC5->C5_OBRA))
   			aadd(aConstr,("."))  //ART: SC5->C5_ART
   		EndIF	
   		IF ! EMPTY(SC5->C5_ZCEI)
		   	aadd(aConstr,(SC5->C5_ZCEI)) //MAX: 23-10-2012 CEI SERA ENVIADO NO LUGAR DA OBRA
   			aadd(aConstr,("."))  //ART: SC5->C5_ART
   		EndIF
		...
		/* MAX ROCHA 03-05-2012: BLOCO ELIMINADO
		...
		Else
			...
			If SC5->( MsSeek( xFilial("SC5") + (cAliasSD2)->D2_PEDIDO) )
			 	if ( SC5->(FieldPos("C5_OBRA")) > 0 .And. !Empty(SC5->C5_OBRA) ) .And. ( SC5->(FieldPos("C5_ARTOBRA")) > 0 .And.;
			 		 !Empty(SC5->C5_ARTOBRA) )
					aadd(aConstr,(SC5->C5_OBRA))
		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
				...
				Else
					cNatOper+=" "
				EndIF
				cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA)+" " //+ALLTRIM(FORMULA("004"))
			EndIf
			...
			If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
				cMensCli +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SC5->C5_MENNOTA)),AllTrim(SC5->C5_MENNOTA))//+ALLTRIM(FORMULA("004"))
			EndIf
			If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
				cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(SC5->C5_MENPAD))),AllTrim(FORMULA(SC5->C5_MENPAD)))//+ALLTRIM(FORMULA("004"))
			EndIf
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If lNatOper .And. !Empty(SC5->C5_MENNOTA)
					...
					Else	
						cNatOper+=" "
					EndIf
					cNatOper += Alltrim(If(FindFunction('CleanSpecChar'),CleanSpecChar(Alltrim(SC5->C5_MENNOTA)),SC5->C5_MENNOTA))+" "//+ALLTRIM(FORMULA("004"))
				EndIf
...
If !Empty(aNota)
	...
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Início FSW-BH - Chamada daFunção para montagem da mensagem integração KP³
	//³Fernando Ferreira - 08/11/2011                                          ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	U_FSMsgCus(@cNatOper, SF2->F2_DUPL, SF2->F2_PREFIXO, SF2->F2_CLIENTE, SF2->F2_LOJA, SF2->F2_DOC,SF2->F2_SERIE)
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Fim FSW-BH - Chamada daFunção para montagem da mensagem integração KP   ³
	//³Fernando Ferreira - 08/11/2011                                          ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
Else
	//MAX/TOPMIX: 21-08-2012 Exigibilidade Suspensa em Guaratingueta, não enviar TAG abaixo;
   IF SF4->F4_ISSST <> '5' .and. SF4->F4_ISSST <> '6'
...
For Nx := 1 to Len(aProd)   
    ...
    EndIf
	cXml += '<DiscriminacaoServico>'+AllTrim(cNatOper+cCargaTrb)+'</DiscriminacaoServico>'

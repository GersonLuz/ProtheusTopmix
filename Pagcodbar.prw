...
Do Case
   Case _cBanco == "237" 	// BRADESCO               
    ...
    _Cagced  := SUBSTR(_Ccodbar,5,4)

    _RETDIG := " "
    _DIG1   := SUBSTR(_Ccodbar,5,1)
    _DIG2   := SUBSTR(_Ccodbar,6,1)
    _DIG3   := SUBSTR(_Ccodbar,7,1)
    _DIG4   := SUBSTR(_Ccodbar,8,1)
    ...
    _RETDIG := IF( _resto == 0,"P",IF(_resto == 1,"P",_DIGITO))
    _Cdigag := _RETDIG
    ...
    _RETDIG := " "
    _DIG1   := SUBSTR(_Ccodbar,24,1)
    _DIG2   := SUBSTR(_Ccodbar,25,1)
    _DIG3   := SUBSTR(_Ccodbar,26,1)
    _DIG4   := SUBSTR(_Ccodbar,27,1)
    _DIG5   := SUBSTR(_Ccodbar,28,1)
    _DIG6   := SUBSTR(_Ccodbar,29,1)
    _DIG7   := SUBSTR(_Ccodbar,30,1)
  ...
  Case _cBanco == "341"		// ITAU
   	// Agencia do cedente
    _Cagced  := SUBSTR(_Ccodbar,18,2)+SUBSTR(_Ccodbar,22,1)

    // Conta-Digito do cedente
    _Ctaced  :=  SUBSTR(_Ccodbar,23,5)
    _Cdigcta :=  SUBSTR(_Ccodbar,28,1)    

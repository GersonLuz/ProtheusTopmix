...
//------------------------------------------------------------------- 
/*/{Protheus.doc} FSFATR02() 
Imprime notas fiscais de serviço
...
Local cDescr3  	:= ""
Local cPict    	:= ""
Local titulo   	:= "Nota Fiscal LayOut 1"

Local imprime  	:= .T.
Local cValPeg		:= "FSFATR02"	 
Local aOrd 		:= {}
...
Private limite      := 80
Private tamanho     := "M"
Private nomeprog    := "FSFATR02" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo       := 18
...
Private CONTFL      := 01
Private m_pag       := 01
Private wnrel       := "FSFATR02" // Coloque aqui o nome do arquivo usado para impressao em disco 
Private lRetQry	  := .T.	
Private nLinRel     := 60
...
MsgRun("Gerando o relatório de notas fiscais de serviço","Por favor, aguarde....",{|| lRetQry := FQryRel(cNotIni,cNotFim,cNotSer)})
...
While TRBCABEC->(!Eof())
    ...
    If nQuePag >= nQtdMens
      @ nLinRel++, 000 PSAY "CEI: " +TRBCABEC->C5_ZCEI
...
@ nLinRel,068 PSAY AllTrim(TRBCABEC->F2_DOC)
...
@ nLinRel,040 PSAY Transform(TRBITENS->D2_PRCVEN , "@E 999,999,999.99") 
@ nLinRel,052 PSAY Transform(TRBITENS->D2_QUANT  , "@E 999,999,999.99") 
...
@ nLinRel++, 020 PSAY TRBCABEC->A1_BAIRRO  //Campo 303
@ nLinRel++, 020 PSAY TRBCABEC->A1_MUN  + " / " + TRBCABEC->A1_EST  + " - " + TRBCABEC->A1_CEP //Campo 304
